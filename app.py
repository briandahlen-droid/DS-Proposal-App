...
with tab1:
    ...
    with col4:
        # Auto-fill from lookup, but allow manual editing
        if 'lookup_address' in st.session_state and not st.session_state.get('project_address'):
            st.session_state['project_address'] = st.session_state['lookup_address']
        if 'lookup_city' in st.session_state and not st.session_state.get('project_city_state_zip'):
            st.session_state['project_city_state_zip'] = f"{st.session_state['lookup_city']}, FL {st.session_state.get('lookup_zip', '')}"
...
# TAB 2: Project Details (Fee Type + Assumptions)
with tab2:
    st.subheader("Fee Type Selection")
    fee_type = st.radio(
...
with tab3:
    ...
    for task_num in sorted(DEFAULT_FEES.keys()):
        ...
        if task_selected:
            ...
            if task_num == '310':
                st.markdown("**📋 Select Construction Phase Services to Include:**")
                
                col_svc1, col_svc2, col_svc3 = st.columns(3)
                
                with col_svc1:
                    include_shop_drawings = st.checkbox("Shop Drawing Review", value=True, key="include_shop_drawings")
                    include_rfi = st.checkbox("RFI Response", value=True, key="include_rfi")
                    include_oac = st.checkbox("OAC Meetings", value=True, key="include_oac")
                
                with col_svc2:
                    include_site_visits = st.checkbox("Site Visits", value=True, key="include_site_visits")
                    include_asbuilt = st.checkbox("As-Built Reviews", value=True, key="include_asbuilt")
                    include_inspection_tv = st.checkbox("Inspection & TV Reports Review", value=False, key="include_inspection_tv")
                
                with col_svc3:
                    include_record_drawings = st.checkbox("Record Drawings (Water/Sewer)", value=False, key="include_record_drawings")
                    include_fdep = st.checkbox("FDEP Clearance Submittals", value=True, key="include_fdep")
                    include_compliance_letter = st.checkbox("Letter of General Compliance", value=True, key="include_compliance")
                    include_wmd_cert = st.checkbox("WMD Certification", value=True, key="include_wmd")
                
                st.markdown("**📝 Service Hours (for selected services):**")
                ...
                with col_hrs1:
                    shop_drawing_hours = st.number_input(
                        ...
                        disabled=not include_shop_drawings
                    )
                ...
                with col_hrs2:
                    ...
                    site_visits = st.number_input(
                        ...
                        value=4,
                        key="site_visits",
                        disabled=not include_site_visits
                    )
                ...
                with col_hrs3:
                    record_drawing_hours = st.number_input(
                        ...
                        disabled=not include_record_drawings
                    )
                ...
                selected_tasks[task_num]['services'] = {
                    'shop_drawings': include_shop_drawings,
                    'rfi': include_rfi,
                    'oac': include_oac,
                    'site_visits': include_site_visits,
                    'asbuilt': include_asbuilt,
                    'inspection_tv': include_inspection_tv,
                    'record_drawings': include_record_drawings,
                    'fdep': include_fdep,
                    'compliance_letter': include_compliance_letter,
                    'wmd_cert': include_wmd_cert
                }
