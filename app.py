...
def lookup_hillsborough_property(parcel_id):
    ...
    for folio_format in formats_to_try:
        ...
        try:
            ...
            if data.get('error'):
                error_details.append(f"API Error for '{folio_format}': {data['error'].get('message', 'Unknown error')}")
                continue
            ...
            if data.get('features') and len(data['features']) > 0:
                ...
                error_details.append(f"No features found for '{folio_format}'")
        
        except requests.exceptions.Timeout:
            error_details.append(f"Timeout for '{folio_format}'")
        except requests.exceptions.ConnectionError:
            error_details.append(f"Connection failed for '{folio_format}'")
        except Exception as e:
            error_details.append(f"Error for '{folio_format}': {str(e)}")
    
    return {
        'success': False,
        'error': f'Parcel not found. Details: {"; ".join(error_details)}'
    }
